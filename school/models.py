...
DEFAULT_PERMISSIONS = {
    'admin': {
        ...
        'inspection_visits': ['view', 'add', 'edit', 'delete'],
        'visit_program': ['view', 'add', 'edit', 'delete'],
        'absence': ['view', 'add'],
        'schedule': ['view', 'add', 'edit', 'delete', 'generate', 'export', 'print', 'manage_constraints', 'manage_settings'],
        'survey': ['view', 'add'],
    ...
    'vice_principal': {
        ...
        'discipline': ['view', 'add'],
        'absence': ['view', 'add'],
        'schedule': ['view', 'add', 'edit', 'generate', 'export', 'print'],
        'reciprocal_visits': ['view', 'add', 'edit', 'delete'],
...
class SchedulePlan(models.Model):
    ...
    name = models.CharField('اسم البرنامج', max_length=150)
    version_label = models.CharField('تسمية الإصدار', max_length=100, blank=True)
    academic_year = models.CharField('العام الدراسي', max_length=50, blank=True)
    semester = models.CharField('الفصل الدراسي', max_length=50, blank=True)
    days = models.JSONField('أيام الدوام', default=list)
    periods = models.JSONField('الحصص', default=list)
    breaks = models.JSONField('الاستراحة', default=list, blank=True)
    status = models.CharField('الحالة', max_length=20, choices=STATUS_CHOICES, default='draft')
    parent = models.ForeignKey('self', null=True, blank=True, on_delete=models.SET_NULL,
                               related_name='children', verbose_name='نسخة من')
    hard_score = models.FloatField('نسبة الالتزام بالقيود الصلبة', null=True, blank=True)
    soft_score = models.FloatField('نسبة الالتزام بالقيود الناعمة', null=True, blank=True)
    ...
    def active_days(self):
        return [d for d in (self.days or []) if d.get('active', True)]

    @property
    def active_periods(self):
        return [p for p in (self.periods or []) if p.get('active', True)]

    @property
    def total_required_periods(self):
        return sum(tl.weekly_periods for tl in self.teaching_loads.all())


class TeachingLoad(models.Model):
    plan = models.ForeignKey(SchedulePlan, on_delete=models.CASCADE,
                              related_name='teaching_loads', verbose_name='البرنامج')
    teacher = models.ForeignKey(Teacher, on_delete=models.CASCADE,
                                related_name='teaching_loads', verbose_name='المعلم')
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=['plan', 'teacher', 'subject', 'student_class'],
                name='unique_teaching_load'),
        ]

    def __str__(self):
        return f'{self.teacher} - {self.subject} - {self.student_class} ({self.weekly_periods})'


class TeacherAvailability(models.Model):
    plan = models.ForeignKey(SchedulePlan, on_delete=models.CASCADE,
                              related_name='availabilities', verbose_name='البرنامج')
    teacher = models.ForeignKey(Teacher, on_delete=models.CASCADE,
                                 related_name='availabilities', verbose_name='المعلم')
    day = models.CharField('اليوم', max_length=20)
    period = models.PositiveIntegerField('رقم الحصة')
    available = models.BooleanField('متاح', default=True)
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=['plan', 'teacher', 'day', 'period'],
                name='unique_teacher_availability'),
        ]

    def __str__(self):
        return f'{self.teacher} - {self.day} ح{self.period}'


class ScheduleConstraint(models.Model):
    TYPE_CHOICES = [('hard', 'إلزامي'), ('soft', 'تفضيلي')]
    plan = models.ForeignKey(SchedulePlan, on_delete=models.CASCADE,
                              related_name='constraints', verbose_name='البرنامج')
    type = models.CharField('النوع', max_length=10, choices=TYPE_CHOICES, default='hard')
    code = models.CharField('الرمز', max_length=60)
    label = models.CharField('الوصف', max_length=200)
...
class FixedLesson(models.Model):
    plan = models.ForeignKey(SchedulePlan, on_delete=models.CASCADE,
                              related_name='fixed_lessons', verbose_name='البرنامج')
    day = models.CharField('اليوم', max_length=20)
    period = models.PositiveIntegerField('رقم الحصة')
    teacher = models.ForeignKey(Teacher, on_delete=models.CASCADE,
                                 related_name='fixed_lessons', verbose_name='المعلم')
    subject = models.ForeignKey(Subject, on_delete=models.CASCADE,
                                 related_name='fixed_lessons', verbose_name='المادة')
    student_class = models.ForeignKey(Class, on_delete=models.CASCADE,
                                       related_name='fixed_lessons', verbose_name='الصف')
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=['plan', 'day', 'period'],
                name='unique_fixed_lesson_cell'),
        ]

    def __str__(self):
        return f'{self.day} ح{self.period} - {self.subject} - {self.student_class}'


class ScheduleEntry(models.Model):
    plan = models.ForeignKey(SchedulePlan, on_delete=models.CASCADE,
                              related_name='entries', verbose_name='البرنامج')
    day = models.CharField('اليوم', max_length=20)
    period = models.PositiveIntegerField('رقم الحصة')
    teacher = models.ForeignKey(Teacher, on_delete=models.CASCADE,
                                 related_name='plan_entries', verbose_name='المعلم')
    subject = models.ForeignKey(Subject, on_delete=models.CASCADE,
                                 related_name='plan_entries', verbose_name='المادة')
    student_class = models.ForeignKey(Class, on_delete=models.CASCADE,
                                       related_name='plan_entries', verbose_name='الصف')
    fixed = models.BooleanField('مثبتة', default=False)
    color = models.CharField('لون', max_length=20, blank=True)
